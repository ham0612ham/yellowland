...
$(function(){
	   ...
	   $("ul.tabs li").click(function() {
	      ...
	      let url = "${pageContext.request.contextPath}/myReply/list";   
	      location.href = url;
	   });
	});
function searchList() {
	...
	<div class="body-container">
		<div class="table2">
					...
					<tbody>
						<c:forEach var="dto" items="${list}" varStatus="status">
							<tr>
								...
								<td style=" max-width: 300px; text-overflow: ellipsis; white-space: nowrap; overflow:hidden;">${dto.subject}</td>
								
								<td>${dto.catejobName}</td>
								<td>${dto.jobName}</td>
								<td>${dto.regDate}</td>
							</tr>
						</c:forEach>
			...
			<div class="board-list-footer d-flex justify-content-between" style="margin-top: 30px;">
				<button class="btnDelete btn btn-light" style="float: left; width: 90px; border-radius: 8px; border: 0.5px solid #A3A6AD; color: #A3A6AD;" onclick="location.href='${pageContext.request.contextPath}/myTypeReply/list';">
					새로고침
				</button>
				<form class="d-flex justify-content-between" name="searchForm"
					class="d-flex justify-content-between"
					action="${pageContext.request.contextPath}/myTypeReply/list"
					method="post">
					<div>
